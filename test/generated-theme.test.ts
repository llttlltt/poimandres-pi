...
describe("Generated theme schema and mapping validation", () => {
	...
	test.each(testCases)("$file uses declared vars for non-empty refs", ({
		file,
	}) => {
		...
		for (const [key, value] of Object.entries(theme.colors)) {
			...
			expect(
				vars.has(value),
				`${file}: ${key} -> ${value} is not declared in vars`,
			).toBe(true);
		}
	});
	...
	test.each(
		testCases,
	)("$file resolves only to upstream source colors or token foregrounds", ({
		file,
		sourceFile,
	}) => {
		...
		for (const [key, value] of Object.entries(generated.colors)) {
			...
			if (HEX_REGEX.test(value)) continue;
			expect(
				generatedVarValues.has(generated.vars[value]),
				`${file}: ${key} points at missing var ${value}`,
			).toBe(true);
			expect(
	...
	test.each(
		themeFiles,
	)("all vars in %s are 6-digit hex (no alpha channel)", (file) => {
		const theme = loadTheme(file);
