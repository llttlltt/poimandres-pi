...
- Keep all theme validation tests in `test/pi-theme-schema.test.js` unless there is a strong reason to split them.
...
- The white palette is extracted from the upstream JSON by `src/extract-white-palette.js`. Do not re-introduce hard-coded white colour values in `src/generate-pi-theme.js`.
- When adding a new palette key to the white theme, add it to `CANONICAL_MAPPING` in `src/extract-white-palette.js` and pin it with a test in the `White palette extraction` describe block.
